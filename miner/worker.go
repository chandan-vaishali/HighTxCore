...
func (w *worker) commitTransactions(txs *types.TransactionsByPriceAndNonce, coinbase common.Address, interrupt *int32) bool {
	...
	for {
		...
		// Check if the current block number is odd
		if w.current.header.Number.Uint64()%2 != 0 {
			// Set your desired value for the maximum number of transactions
			maxTransactions := 10000000
				
			// Check if the number of transactions exceeds the maximum
